...
# Download if not exists (we know it exists from previous step, but good practice)
...
# We will split by this pattern to get chunks
$chunks = $content -split '(?=<p><strong>\d+\.)'

# The first chunk is header junk, skip it
$chunks = $chunks | Select-Object -Skip 1

$questionsData = @()

foreach ($chunk in $chunks) {
    # 1. Extract Question Text and Number
...
# Create HTML Template using single quotes to preserve JS template literals
$htmlTemplate = @'
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        ...
        .option-item.missed { border-color: #28a745; border-style: dashed; } /* Correct answer not selected */
...
<body>
    ...
    <script>
        ...
        let currentIdx = 0;
        let score = 0;
        let userAnswers = {}; // Store user state: { qId: { selected: [indices], checked: boolean } }
        ...
        function renderQuestion() {
            ...
            q.options.forEach((opt, idx) => {
                ...
                // Click handler only if not checked
